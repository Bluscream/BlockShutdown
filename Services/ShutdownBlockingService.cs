...
public class ShutdownBlockingService : ApplicationContext
{
    ...
    private EventDirectoryService _eventDirectoryService;
    ...
    public ShutdownBlockingService(Configuration config)
    {
        ...
        _eventDirectoryService = new EventDirectoryService();
        if (_config.EnableEventDirectories)
            _eventDirectoryService.Initialize();
    ...
    public bool HandleShutdownEvent(uint wParam, uint lParam)
    {
        ...
        {
            if (isLogoff)
                _eventDirectoryService.ExecuteEvent("LogoffRequested");
            else
                _eventDirectoryService.ExecuteEvent("ShutdownRequested");
        }
        ...
        if (!_blockShutdown)
        {
            ...
            {
                if (isLogoff)
                    _eventDirectoryService.ExecuteEvent("LogoffRequestAccepted");
                else
                    _eventDirectoryService.ExecuteEvent("ShutdownRequestAccepted");
            }
        ...
        if (_askForConfirmation)
        {
            ...
            else
            {
                ...
                {
                    if (isLogoff)
                        _eventDirectoryService.ExecuteEvent("LogoffRequestBlocked");
                    else
                        _eventDirectoryService.ExecuteEvent("ShutdownRequestBlocked");
                }
